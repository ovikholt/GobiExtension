...
TOKEN=$(curl "https://accounts.google.com/o/oauth2/token" -d "client_id=$CLIENT_ID&code=$CODE&grant_type=authorization_code&redirect_uri=urn:ietf:wg:oauth:2.0:oob" | jq '.access_token')
# {
...
FILE_NAME=../GobiInsertStories.zip

APP_ID=fimchdpaiaeiflignlidhiabhnijcjki

curl -H "Authorization: Bearer $TOKEN" -H "x-goog-api-version: 2" -X PUT -T $FILE_NAME https://www.googleapis.com/upload/chromewebstore/v1.1/items/$APP_ID
